...
$(function(){
    var bMainIndex = $('body').find('.b-main--index');

    var bMainIndex__showHiddenCorners = function(obj, state){
        $(obj).each(function(){
            var trigger = $(this).find('.js-trigger');
            var content = $(this).find('.js-changed');
            $(this).mouseenter(function(){
                $(bMainIndex).addClass('state-' +state);
                $(trigger).addClass('is-hidden');
                $(content).removeClass('is-hidden');
            });
            $(this).mouseleave(function(){
                $(bMainIndex).removeClass('state-1').removeClass('state-2');
                $(trigger).removeClass('is-hidden');
                $(content).addClass('is-hidden');
            });
        });
    };
    bMainIndex__showHiddenCorners($('.b-main--index__address'), 1);
    bMainIndex__showHiddenCorners($('.b-main--index__tip'), 2);
